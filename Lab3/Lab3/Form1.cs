...
namespace Lab3
{
    public partial class Form1 : Form
    {
        ...
        private void LoadListView(List<SinhVien> dssv)
        {
            ...
            //statusstr
            int count;
            count = this.lvSinhVien.Items.Count;

            sTTS_TongSV.Text = "Tổng số sinh viên " + count;
        }
        ...
        private void tìmKiếmToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            frm_TuyChon frm = new frm_TuyChon(TrangThai.timKiem);
            frm.ShowDialog();
            if (frm.DialogResult == DialogResult.OK)
            {
                if (frm.trangthai == TrangThai.MaSV)
                {
                    list = qlsv.DanhSach.FindAll(x => x.MaSo == frm.search).ToList();
                }
                else if (frm.trangthai == TrangThai.Ten)
                {
                    list = qlsv.DanhSach.FindAll(x => x.HoTen == frm.search).ToList();
                }
                else 
                {
                    list = qlsv.DanhSach.FindAll(x => x.NgaySinh == DateTime.Parse(frm.search)).ToList();
                }
                if (list.Count==0)
                {
                    MessageBox.Show("Không tim thấy sinh viên", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    MessageBox.Show("Số sinh viên tìm thấy: " + list.Count);
                    LoadListView(list);
        ...
        {
            List<SinhVien> list = new List<SinhVien>();
            frm_TuyChon frm = new frm_TuyChon(TrangThai.sapXep);
            if (frm.ShowDialog() == DialogResult.OK)
            {
                if (frm.trangthai == TrangThai.MaSV)
                {
                    list = qlsv.DanhSach.OrderBy(x => x.MaSo).ToList();
                }
                else if (frm.trangthai == TrangThai.Ten)
                {
                    list = qlsv.DanhSach.OrderBy(x => x.HoTen).ToList();
                }
                else
                {
                    list = qlsv.DanhSach.OrderBy(x => x.NgaySinh ).ToList();
                }
